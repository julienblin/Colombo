...
namespace Colombo.Impl
{
    public class MessageBus : IMessageBus
    {
            ...
            where TResponse : Response, new()
        {
            ...
            IColomboSingleInvocation topInvocation = BuildSingleInvocationChain(request, messageProcessor);
        ...
        private IColomboSingleInvocation BuildSingleInvocationChain(BaseRequest request, IMessageProcessor messageProcessor)
        {
        ...
        private Response[] InternalParallelSend(params BaseRequest[] requests)
        {
            ...
            var topInvocation = BuildParallelInvocationChain(requests);
            topInvocation.Proceed();
            return topInvocation.Responses;
        }

        private IColomboParallelInvocation BuildParallelInvocationChain(BaseRequest[] requests)
        {
            Contract.Assume(requests != null);
            Contract.Assume(messageProcessors != null);
            Contract.Assume(MessageBusSendInterceptors != null);

            var parallelSendInvocation = new ParallelSendActionsColomboParallelInvocation(messageProcessors);
            parallelSendInvocation.Logger = Logger;
            IColomboParallelInvocation currentInvocation = parallelSendInvocation;
            foreach (var interceptor in MessageBusSendInterceptors.Reverse())
            {
                if (interceptor != null)
                    currentInvocation = new InterceptorColomboParallelInvocation(interceptor, currentInvocation);
            }
            currentInvocation.Requests = requests;
            return currentInvocation;
        }
